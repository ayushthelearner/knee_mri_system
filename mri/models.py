...
class CustomUser(AbstractUser):
    _id = models.ObjectIdField(primary_key=True)  # Explicitly set _id as primary key


from django.conf import settings  # Import settings for dynamic user reference

class MRIImage(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="mri_images", null=True, blank=True) 
    image = models.ImageField(upload_to='uploads/')
    uploaded_at = models.DateTimeField(auto_now_add=True)
    result = models.TextField(blank=True, null=True)
...
class Hospital(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="hospital", null=True, blank=True)
    name = models.CharField(max_length=255, unique=True, default="Unknown Hospital")
    address = models.TextField(default="Not provided")
    phone_number = models.CharField(max_length=15, default="0000000000")
    registration_number = models.CharField(max_length=50, unique=True, default="REG123456")
